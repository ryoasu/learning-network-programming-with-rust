...
impl DhcpPacket {
  ...
  pub fn set_chaddr(&mut self, chaddr: MacAddr) {
    ...
    // chaddrフィールドは16オクテット確保されているが、今回はMACアドレスしか入らないため、6オクテット分確保している
  ...
  pub fn get_option(&self, option_code: u8) -> Option<Vec<u8>> {
    ...
    while options[index] != OPTION_END {
      if options[index] == option_code {
        // 目的のコードだった場合。データはコード、サイズ、データの順に並ぶ。
        // ▼ e.g. Subnet Mask
...
impl DhcpServer {
  pub fn new() -> Result<DhcpServer> {
    ...
    info!("network_addr_with_prefix: {}", network_addr_with_prefix);
