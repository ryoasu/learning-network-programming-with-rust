...
fn dhcp_handler(packet: &DhcpPacket, soc: &UdpSocket, dhcp_server: Arc<DhcpServer>) -> Result<()> {
    let message = packet
        ...
        .ok_or_else(|| anyhow!("specified option was not found"))?;
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    let server_ip =
        util::u8_to_ipv4addr(&server_id).ok_or_else(|| anyhow!("Failed to convert ip addr."))?;
    ...
    let ip_bin = received_packet
        ...
        .unwrap();
    let ip_to_be_leased =
        util::u8_to_ipv4addr(&ip_bin).ok_or_else(|| anyhow!("Failed to convert ip addr."))?;
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_ip) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        ...
        let requested_ip = util::u8_to_ipv4addr(&requested_ip)
            .ok_or_else(|| anyhow!("Failed to convert ip addr."))?;
...
fn select_lease_ip(
    ...
    while let Some(ip_addr) = dhcp_server.pick_available_ip() {
        ...
        // info!("check: {}", util::is_ipaddr_available(ip_addr).is);
        // match util::is_ipaddr_available(ip_addr) {
        //     Ok(_) => info!("OK"),
        //     Err(e) => info!("check: {}", e),
        // };
